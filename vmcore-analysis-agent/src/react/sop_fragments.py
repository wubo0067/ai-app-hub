...
- If struct -o <guessed_type> fails on a module crash path, do not keep guessing private types. Instead: (1) load module symbols with `mod -s <module>` first; (2) if symbol enumeration is then necessary, emit it as `run_script` with a module-scoped, grep-bounded command (e.g., `sym -l <module> | grep -i <keyword>`); never run unbounded `sym -l` outside of run_script.
