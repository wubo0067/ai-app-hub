...
When a syscall-path backtrace remains coherent up to the interrupted site, you must analyze that active non-exception chain before promoting exception-handler frames to suspects. For example, if the live path is sys_open -> do_filp_open -> path_openat -> do_last -> link_path_walk -> inode_permission, first inspect those active VFS/open-path functions for local objects, copy primitives, and stack layout. Do not jump directly to handle_mm_fault or fault.c merely because exception-handler addresses also appear on the stack.
