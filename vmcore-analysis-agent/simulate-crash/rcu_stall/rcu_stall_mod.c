...
static int rcu_stall_thread(void *data) {
  ...
  pr_info("rcu_stall_mod: Starting stall simulation for %d ms, Thread PID: %d, "
  ...
  // 使用 mdelay 进行忙等待，它会持续占用 CPU 时间而不让出处理器，这通常也会触发
  // softlockup 警告。RCU 检测器会在 rcu_cpu_stall_timeout 秒后触发警告或
  // panic。
