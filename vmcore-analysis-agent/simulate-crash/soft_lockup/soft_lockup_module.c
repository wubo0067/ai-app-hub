...
*/
		...
		while (enable_soft_lockup && !kthread_should_stop()) {
			...
			while (counter++ < 1000000) {
				/*
				如果改为 cpu_relex(),实际上会减少 CPU 的负载，因为它允许 CPU
				在忙等待期间节省能源或优化执行。这与软锁模拟的目标（让 CPU
				持续繁忙）相矛盾。

				barrier, 不会产生实际的 CPU 指令
				Prevent compiler optimization，来消耗 CPU 周期
				如果没有这行代码，编译器可能会优化掉这个循环，从而无法模拟软锁死
				*/
				barrier();
			}
	...
	printk(KERN_INFO
	       "Soft Lockup Module: Stopping soft lockup simulation\n");
	return 0;
}
...
static int __init soft_lockup_init(void)
{
	...
	soft_lockup_task = kthread_run(soft_lockup_simulation, NULL,
				       "soft_lockup_kthread");
	if (IS_ERR(soft_lockup_task)) {
		printk(KERN_ERR
		       "Soft Lockup Module: Failed to create kernel thread\n");
