...
MODULE_PARM_DESC(cpu_number, "CPU number to run soft lockup on (-1 for any CPU)");

static bool enable_soft_lockup = false;
module_param(enable_soft_lockup, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(enable_soft_lockup, "Enable soft lockup simulation (default: false)");

// Soft lockup simulation function
static int soft_lockup_simulation(void *data)
{
    printk(KERN_INFO "Soft Lockup Module: Starting soft lockup simulation on CPU %d\n", smp_processor_id());
    ...
    while (!kthread_should_stop() && enable_soft_lockup)
    {
        ...
        /*
        在 preempt_disable() 状态下，CPU 仍然会响应硬件中断（如时钟中断、网卡中断、IPI 核间中断）。
...
{
    printk(KERN_INFO "Soft Lockup Module: Initializing soft lockup simulation module\n");

    if (!enable_soft_lockup)
    {
        printk(KERN_WARNING "Soft Lockup Module: Soft lockup disabled by default. Set enable_soft_lockup=1 to activate.\n");
        return 0;
    }

    // Create a kernel thread for soft lockup simulation
    soft_lockup_task = kthread_run(soft_lockup_simulation, NULL, "soft_lockup_kthread");
    ...
    {
        if (set_cpus_allowed_ptr(soft_lockup_task, cpumask_of(cpu_number)))
        {
            printk(KERN_WARNING "Soft Lockup Module: Failed to bind to CPU %d, running on any CPU\n", cpu_number);
        }
        else
        {
    ...
    printk(KERN_INFO "Soft Lockup Module: Loaded successfully. Soft lockup simulation started.\n");
    return 0;
}
