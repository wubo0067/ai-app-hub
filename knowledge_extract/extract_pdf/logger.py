...
- 文件：output/extract_pdf.log，记录 DEBUG 及以上（含时间戳/级别），
  完整保留流程细节，便于事后调试与跟踪。
- 每次进程启动会在日志文件里写入一条 "新运行" 分隔行，区分不同次的运行。

用法：
    from logger import get_logger, parse_level
    log = get_logger()                          # 模块级获取（默认控制台 INFO）
    get_logger(parse_level(args.log_level))     # 按命令行参数调整控制台级别
"""
...
LOG_DIR = Path(__file__).resolve().parent / "output"   # 日志目录（与 notes.md 同级）
LOG_FILE = LOG_DIR / "extract_pdf.log"                 # 日志文件路径
