...
- **🔍 错题库检索** — 按学科切换、按知识点标签筛选、按关键词搜索已索引的错题（搜索范围包括标题、题目内容、标签、备注），支持日期范围筛选
...
| POST | `/api/images/index` | 将图片标记为已索引（自动推断学科，接收 title/content/tags 等） |
| PUT | `/api/images/update` | 更新已索引图片的元数据（含解答、掌握程度、备注等） |
| DELETE | `/api/images/delete` | 从索引中移除图片（参数：`file_path`） |
