...
MATH_KNOWLEDGE_POINTS = [
    ...
    "胡不归", "圆的内接四边形", "8 字相似模型", "反 8 相似模型",
    "两定一动求最值，定线段，构造平行四边形", "两定一动，将军饮马"
]

ANALYSIS_PROMPT = (
    '你是一位经验丰富的初中数学老师。请分析图片中的这道数学题，识别其所涉及的知识点。\n'
    '只输出一个 JSON 数组，不要有任何其他文字，不要用 markdown 代码块包裹，格式如下：\n'
    '["知识点 1", "知识点 2", "知识点 3"]\n'
    f'知识点标签要具体、准确，覆盖这道题真正用到的所有中学数学考点（例如：{", ".join(MATH_KNOWLEDGE_POINTS)} 等），一般给 3 到 6 个，不要过于笼统（避免只写"几何"这种大类）。'
)
